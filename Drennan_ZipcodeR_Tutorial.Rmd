...
ca_data <- read.csv("Data/property_shark_data_2022.csv")%>%
  filter(State == "CA")

ca_data <- reverse_zipcode(data$Zip.Code)

ca_data <- st_as_sf(ca_data, coords = c("lng", "lat"), crs = 4326)
...
ggplot()+
  geom_sf(data = states)+
  geom_sf(data = ca_data)+
  geom_point(size = "median_home_value)
